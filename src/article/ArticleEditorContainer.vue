...
import { ref } from 'vue'
import ArticleClient, { type ArticleData } from '../clientold/ArticleClient'
...
const save = async (article: ArticleData) => {
  ...
  if (isExistingArticle.value) {
    result = await ArticleClient.updateArticle(articleData.value.slug ?? '', {
      article: newArticle,
    }).promise
  } else {
...
<template>
  <ArticleEditor
    ...
    :is-existing-article="isExistingArticle"
